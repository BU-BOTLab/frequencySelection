...
for sepIdx = 1:length(sep)
    ...
    for f=1:length(freqList)
        ...
        parfor i=1:numTrials
            ...
            recOP = fitMu_newOpts([noiseReal;noiseImag],wt, fa', nind, sep(sepIdx),'test');
...
%%
    %Save MAT FILES
    save(sprintf('../generatedData/Digital_noise_%dnm_unequalData_optimized_v2.mat',wavelength),'saveMuaTrue','saveMusTrue', 'saveMuaRec','saveMusRec','freqList');
...
sgtitle('Digital noise model unequal data error single freq optimized')
it = 1;
for thisFreqIdx = 1:3
...
print('../plots/ErrorLandscapeSingleFreq_digital_850nm_unequal_optimized_v3.png','-dpng')


f=figure;
set(f,'Position',[230,260,1470,900])
sgtitle('Digital noise model unequal data error multi-freq optimized')
it = 1;
for thisFreqIdx = 4:6
    for thisSepIdx = 1:length(sepRange)
        subplot(3,3,it)
...
print('../plots/ErrorLandscapeMultiFreq_digital_850nm_unequal_optimized_v3.png','-dpng')